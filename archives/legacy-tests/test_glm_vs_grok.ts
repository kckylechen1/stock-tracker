...
const SYSTEM_PROMPT =
  "你是一位专业的A股投资分析师，擅长分析科技股和成长股。请给出直接、有操作性的建议。";
...
async function main() {
  ...
  if (glmResult.error) {
    ...
    console.log(
      `📊 Token: prompt=${glmResult.tokens.prompt}, completion=${glmResult.tokens.completion}`
    );
    console.log(`\n📝 回复:\n${"-".repeat(50)}`);
  ...
  if (grokResult.error) {
    ...
    console.log(
      `📊 Token: prompt=${grokResult.tokens.prompt}, completion=${grokResult.tokens.completion}`
    );
    console.log(`\n📝 回复:\n${"-".repeat(50)}`);
  ...
  console.log(`|----------|-------------------|-------------------|`);
  console.log(
    `| 耗时     | ${glmResult.duration.toFixed(2).padEnd(17)}s | ${grokResult.duration.toFixed(2).padEnd(17)}s |`
  );
  console.log(
    `| Token    | ${String(glmResult.tokens.total).padEnd(17)} | ${String(grokResult.tokens.total).padEnd(17)} |`
  );
  console.log(
    `| 状态     | ${(glmResult.error ? "❌ 失败" : "✅ 成功").padEnd(17)} | ${(grokResult.error ? "❌ 失败" : "✅ 成功").padEnd(17)} |`
  );
