...
async function testModel(
  ...
  while (iteration < maxIterations) {
    ...
    if (data.error) {
      ...
      return {
        response: `错误: ${data.error.message}`,
        toolsCalled,
...
async function main() {
  ...
  try {
    ...
    console.log(
      `   🔧 工具调用: ${grokResult.toolsCalled.length > 0 ? grokResult.toolsCalled.join(", ") : "无"}`
    );
  ...
  try {
    ...
    console.log(
      `   🔧 工具调用: ${deepseekResult.toolsCalled.length > 0 ? deepseekResult.toolsCalled.join(", ") : "无"}`
    );
  ...
  console.log("├─────────────────┼────────────────────┼────────────────────┤");
  console.log(
    `│ 耗时            │ ${String((grokResult.timeMs / 1000).toFixed(1) + "s").padEnd(18)} │ ${String((deepseekResult.timeMs / 1000).toFixed(1) + "s").padEnd(18)} │`
  );
  console.log(
    `│ 工具调用数      │ ${String(grokResult.toolsCalled.length).padEnd(18)} │ ${String(deepseekResult.toolsCalled.length).padEnd(18)} │`
  );
  console.log(
    `│ 回答长度        │ ${String(grokResult.response.length + " 字符").padEnd(18)} │ ${String(deepseekResult.response.length + " 字符").padEnd(18)} │`
  );
  console.log("└─────────────────┴────────────────────┴────────────────────┘");
