...
export class EnhancedSellSignalDetector {
    ...
    async analyzeSellSignals(
        ...
        // 3. 确定风险等级和行动建议
        const { riskLevel, action, reason } = this.determineAction(sellSignals, totalScore, currentPrice, launchLow);
    ...
    private detectEmptyArrangement(technical: any): SellSignal[] {
        ...
        if (technical.ma5 < technical.ma10 && technical.ma10 < technical.ma20 && technical.ma20 < technical.ma60) {
            signals.push({
                type: 'sell',
                signal: '空头排列',
    ...
    private detectVolumeShrinkage(technical: any): SellSignal[] {
        ...
        // 计算平均成交量
        const avgVolume = technical.volumes.reduce((a: number, b: number) => a + b, 0) / technical.volumes.length;
     ...
     * 检测止损信号
     */
    private detectStopLossSignals(currentPrice: number, launchLow: number): SellSignal[] {
        const signals: SellSignal[] = [];
        ...
        } else if (currentPrice < launchLow * 1.05) { // 接近启动低点5%以内
            signals.push({
                type: 'warning',
                signal: '接近止损线',
    ...
    private determineAction(
        ...
        currentPrice: number,
        launchLow: number
    ): { riskLevel: 'low' | 'medium' | 'high' | 'extreme', action: 'hold' | 'reduce' | 'sell' | 'stop_loss', reason: string } {
        ...
        // 计算风险评分
        const riskScore = sellSignals.reduce((sum, signal) => sum + Math.abs(signal.scoreImpact), 0);
...
export async function quickSellSignalCheck(
    ...
    return await detector.analyzeSellSignals(symbol, currentPrice, launchLow, totalScore);
}
...
export function formatSellSignalAnalysis(analysis: SellSignalAnalysis): string {
    ...
    if (analysis.sellSignals.length > 0) {
        ...
        analysis.sellSignals.forEach((signal, idx) => {
            const icon = signal.type === 'stop_loss' ? '🛑' : signal.type === 'sell' ? '❌' : '⚠️';
