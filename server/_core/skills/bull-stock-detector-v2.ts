...
export class BullStockDetectorV2 {
    ...
    for (let i = 0; i < stockList.length; i += batchSize) {
      ...
      for (const result of batchResults) {
        if (
          ...
          result.value &&
          result.value.totalScore >= this.options.minScore
        ) {
      ...
      console.log(
        `✅ 已分析 ${Math.min(i + batchSize, stockList.length)}/${stockList.length} 只股票`
      );
    }
  ...
  private async getMarketStocks(): Promise<string[]> {
    try {
      // 获取沪深主板股票
      const result = await executeStockTool("search_stock", {
        keyword: "000001",
      });
  ...
  private async analyzeStock(
    symbol: string
  ): Promise<BullStockOpportunity | null> {
    try {
      // 1. 获取基本信息
      ...
      // 3. 获取资金流向（移除北向资金依赖）
      const fundFlowResult = await executeStockTool("get_fund_flow", {
        code: symbol,
      });
      ...
      // 7. 检测卖出信号（使用增强版检测器）
      const sellSignalAnalysis =
        await this.sellSignalDetector.analyzeSellSignals(
          symbol,
          quote.price,
      ...
      const scores = this.calculateComprehensiveScore(
        technical,
        fundFlow,
        signals,
        sellSignals
  ...
  private detectBuySignals(
    ...
    // 均线多头排列
    if (
      technical.ma.ma5 > technical.ma.ma10 &&
      technical.ma.ma10 > technical.ma.ma20 &&
      technical.ma.ma20 > technical.ma.ma60
    ) {
      signals.push("多头排列");
    }
    ...
    // 资金流入
    if (fundFlow?.netInflow > 10000000) {
      // 1000万
  ...
  private calculateComprehensiveScore(
    ...
    // 资金评分（移除北向资金依赖）
    if (fundFlow?.netInflow > 20000000)
      fundScore += 30; // 2000万 - 提高权重
 ...
 * 快速扫描牛股
 */
export async function quickBullStockScan(
  options: BullStockDetectorOptions = {}
): Promise<BullStockOpportunity[]> {
 ...
 * 格式化输出结果
 */
export function formatBullStockResults(
  opportunities: BullStockOpportunity[]
): string {
  ...
  output +=
    "| 排名 | 股票代码 | 股票名称 | 启动日期 | 当前涨幅 | 综合评分 | 风险等级 | 建议行动 | 主要信号 |\n";
  output +=
    "|-----|---------|---------|---------|---------|---------|---------|---------|---------|\n";
