...
export async function* smartStreamChat(
    ...
    const preloadedContext = stockCode && stockContext
    ...
    try {
        ...
        if (!hasContent) {
            ...
            const followUps = generateFollowUpSuggestions(fullContent, lastUserMessage, stockCode);
...
function generateFollowUpSuggestions(
    ...
    if (aiResponse.includes('板块') || aiResponse.includes('概念')) {
        // 板块相关话题
        const sectorMatch = aiResponse.match(/([\u4e00-\u9fa5]+概念|[\u4e00-\u9fa5]+板块)/);
        if (sectorMatch) {
    ...
    if (aiResponse.includes('资金') || aiResponse.includes('净流入') || aiResponse.includes('主力')) {
        suggestions.push('主力资金流入最多的是哪些股票？');
        suggestions.push('如何判断主力是在吸筹还是出货？');
    }

    if (aiResponse.includes('技术') || aiResponse.includes('K线') || aiResponse.includes('均线') || aiResponse.includes('MACD')) {
        suggestions.push('MACD和KDJ哪个更适合短线操作？');
        suggestions.push('怎么用均线判断趋势？');
    }

    if (aiResponse.includes('风险') || aiResponse.includes('注意') || aiResponse.includes('警惕')) {
...
function extractKeyTopics(aiResponse: string, userQuestion: string): string[] {
    ...
    // 提取股票名称
    const stockNames = aiResponse.match(/[\u4e00-\u9fa5]{2,4}(?:股份|科技|集团|电子|通信|医药|新能源)/g);
    if (stockNames) {
