...
function detectFlagPattern(klines: KlineData[]): PatternResult | null {
    ...
    const breakout = lastClose > prevAvgClose * 1.02 && lastVolume > avgVolLast * 1.5;
...
function detectMASqueezePattern(klines: KlineData[]): PatternResult | null {
    ...
    const breakout = lastClose > lastMA5 && lastClose > lastMA10 && lastVolume > avgVolume * 1.2;
 ...
 * @param symbol 股票代码
 */
export async function analyzeMinutePatterns(symbol: string): Promise<MinuteAnalysisResult | null> {
    try {
        // 获取5分钟K线数据
        ...
        let summary = '';
        if (pattern) {
            const stars = '★'.repeat(pattern.strength) + '☆'.repeat(5 - pattern.strength);
...
export function formatMinuteAnalysis(result: MinuteAnalysisResult): string {
    ...
    if (morningKlines.length > 0) {
        ...
        const morningAvgVol = morningVolumes.reduce((a, b) => a + b, 0) / morningVolumes.length;
...
└─ 无均线粘合发散`;
    ...
    // 综合结论（硬编码规则）
    let conclusion = '';
    if (result.pattern && result.pattern.volume_confirmed && result.pattern.strength >= 4) {
