...
export function addPosition(position: Position): boolean {
    ...
    const existingIdx = memory.positions.findIndex(p => p.symbol === position.symbol);
    if (existingIdx >= 0) {
 ...
 * 更新交易结果（事后评价）
 */
export function updateTradeOutcome(tradeId: string, outcome: 'good' | 'bad' | 'neutral', lessonsLearned?: string): boolean {
    const memory = loadMemory();
 ...
 * 获取相关教训（按股票和信号模式匹配）
 */
export function getRelevantLessons(symbol?: string, signals?: { rsi?: number; macd?: string }): TradingLesson[] {
    const memory = loadMemory();
    ...
    return memory.lessons.filter(lesson => {
        ...
        // 按信号模式匹配
        if (signals) {
            if (lesson.signalPattern.includes('RSI<30') && signals.rsi && signals.rsi < 30) {
                return true;
            }
            if (lesson.signalPattern.includes('MACD金叉') && signals.macd === 'golden') {
                return true;
            }
            if (lesson.signalPattern.includes('MACD死叉') && signals.macd === 'dead') {
                return true;
            }
        }
...
export function generateAIContext(currentSymbol?: string): string {
    ...
    if (relevantTrades.length > 0) {
        ...
        for (const t of relevantTrades.slice(-5)) {
            ...
            context.push(`- ${t.date}: ${t.action.toUpperCase()} ${t.name} ${t.price}元 ${t.shares}股 ${emoji}`);
            if (t.lessonsLearned) {
    ...
    if (memory.profile) {
        ...
        if (memory.profile.successPatterns.length) {
            context.push(`- ✅ 成功模式: ${memory.profile.successPatterns.join(', ')}`);
        }
    }
