...
async function getYearStartPrice(symbol: string): Promise<number> {
    try {
        ...
        const yearStart = history.find(k => 
            k.date.startsWith('2025-01') || 
            k.date.startsWith('2025-02')
        );
...
async function main() {
    console.log('╔════════════════════════════════════════════════════════════════════════════════╗');
    console.log('║   2025年涨幅最大的30只股票（市值>200亿）                               ║');
    console.log('╚═══════════════════════════════════════════════════════════════════════════╝\n');
    ...
    top30.forEach((stock, idx) => {
        console.log(`${(idx + 1).toString().padStart(2)}. ${stock.name}(${stock.symbol})`);
        ...
        console.log(`   年度涨幅: ${stock.yearGain > 0 ? '+' : ''}${stock.yearGain.toFixed(2)}%\n`);
    });
