...
async function refreshAccessToken() {
  ...
  if (tokenRefreshLock) {
    console.log("[iFinD] Token refresh already in progress, waiting...");
    // Wait for existing refresh to complete
    while (tokenRefreshLock) {
      await new Promise(resolve => setTimeout(resolve, 100));
    }
    return ENV.ifindAccessToken ? ENV.ifindAccessToken : null;
  }

  tokenRefreshLock = true;
  try {
    console.log("[iFinD] Refreshing access token...");
    const response = await axios.post(
      `${BASE_URL}/api/v1/refresh_access_token`,
      { refresh_token: ENV.ifindRefreshToken },
      { headers: { "Content-Type": "application/json" } }
    );

    if (response.data.code === 0 && response.data.data?.access_token) {
      const newToken = response.data.data.access_token;
      console.log("[iFinD] Token refreshed successfully");
      // update in memory (Note: in a real app, should write back to .env or DB)
      ENV.ifindAccessToken = newToken;
      return newToken;
    } else {
      console.error("[iFinD] Refresh token failed:", response.data);
      return null;
    }
  } catch (error: any) {
    console.error("[iFinD] Refresh token error:", error.message);
    return null;
  } finally {
...
async function callApi(endpoint: string, data: any) {
  try {
    ...
    // 处理 Token 过期 (-1302: Access_Token is expired or ilegal)
    if (response.data.errorcode === -1302) {
      console.warn("[iFinD] Token expired, attempting refresh...");
...
export async function getRealTimeQuotation(codes: string, indicators: string) {
  try {
    const data = await callApi("/api/v1/real_time_quotation", {
      codes,
      indicators,
    });

    if (data.errorcode !== 0) {
      throw new Error(data.errmsg || "iFinD API error");
    }
...
export async function getHistoryData(
  ...
  try {
    const data = await callApi("/api/v1/history", {
      codes,
      indicators,
      startdate,
      enddate,
...
export async function getHighFrequencyData(
  ...
  try {
    const data = await callApi("/api/v1/high_frequency", {
      codes,
      indicators,
      starttime,
      endtime,
...
export async function getTickData(
  ...
  try {
    const data = await callApi("/api/v1/tick", {
      codes,
      indicators,
      starttime,
      endtime,
...
export const INDICATORS = {
  // 基础行情 - iFinD 正确的指标名
  QUOTE:
    "latest,open,high,low,preClose,volume,amount,change,pctChange,turnoverRatio,pe_ttm,pb,totalMarketValue,floatMarketValue,volumeRatio",
...
export async function getStockQuote(code: string) {
  try {
    ...
    const change =
      getValue("change") ?? (price && preClose ? price - preClose : null);
    const pctChange =
      getValue("pctChange") ??
      (change && preClose ? (change / preClose) * 100 : null);
