...
async function callGrokAPI(systemPrompt: string, userMessage: string, useTools: boolean = true) {
    const messages = [
...
async function runTest() {
    ...
    for (const testCase of TEST_CASES) {
        ...
        try {
            ...
            console.log(`\n   ⏱️  耗时: ${elapsed}s`);
            console.log(`   🔧 工具调用: ${toolCalls.length > 0 ? toolCalls.map((t: any) => t.function?.name).join(', ') : '无'}`);
