...
async function runSingleComparison(testCase: TestCase): Promise<{ old: TestResult; new: TestResult }> {
    console.log(`\n${'─'.repeat(70)}`);
    ...
    if (oldResult.success) {
        console.log(`   ✅ 成功 | 耗时: ${(oldResult.latency / 1000).toFixed(2)}s | 输出: ${oldResult.outputLength} 字符`);
        console.log(`   📄 摘要: ${oldResult.output.slice(0, 150).replace(/\n/g, ' ')}...`);
    } else {
    ...
    if (newResult.success) {
        console.log(`   ✅ 成功 | 耗时: ${(newResult.latency / 1000).toFixed(2)}s | 输出: ${newResult.outputLength} 字符`);
        console.log(`   📄 摘要: ${newResult.output.slice(0, 150).replace(/\n/g, ' ')}...`);
    } else {
...
async function main() {
    ...
    const summarize = (results: TestResult[], name: string) => {
        ...
        const avgLatency = successResults.length > 0
            ...
            : 0;
        const avgOutputLength = successResults.length > 0
            ? successResults.reduce((acc, r) => acc + r.outputLength, 0) / successResults.length
            : 0;
    ...
    console.log('\n\n📋 详细对比表\n');
    console.log('| 测试用例   | 旧架构状态 | 旧架构耗时 | 旧架构输出 | 新架构状态 | 新架构耗时 | 新架构输出 |');
    console.log('|------------|-----------|-----------|-----------|-----------|-----------|-----------|');

    for (const result of allResults) {
        const oldStatus = result.old.success ? '✅' : '❌';
        const newStatus = result.new.success ? '✅' : '❌';
        const oldLatency = result.old.success ? `${(result.old.latency / 1000).toFixed(1)}s` : 'N/A';
        const newLatency = result.new.success ? `${(result.new.latency / 1000).toFixed(1)}s` : 'N/A';
        ...
        console.log(`| ${result.old.testCase.padEnd(10)} | ${oldStatus.padEnd(9)} | ${oldLatency.padEnd(9)} | ${oldLen.padEnd(9)} | ${newStatus.padEnd(9)} | ${newLatency.padEnd(9)} | ${newLen.padEnd(9)} |`);
    }
    ...
    // 速度对比
    const speedDiff = ((oldStats.avgLatency - newStats.avgLatency) / oldStats.avgLatency * 100);
    if (newStats.avgLatency < oldStats.avgLatency) {
        console.log(`⚡ 速度: 新架构快 ${Math.abs(speedDiff).toFixed(1)}% (${(newStats.avgLatency / 1000).toFixed(2)}s vs ${(oldStats.avgLatency / 1000).toFixed(2)}s)`);
    } else {
        console.log(`⚡ 速度: 旧架构快 ${Math.abs(speedDiff).toFixed(1)}% (${(oldStats.avgLatency / 1000).toFixed(2)}s vs ${(newStats.avgLatency / 1000).toFixed(2)}s)`);
    }

    // 输出质量
    if (newStats.avgOutputLength > oldStats.avgOutputLength) {
        console.log(`📝 输出详细度: 新架构更详细 (${newStats.avgOutputLength.toFixed(0)} vs ${oldStats.avgOutputLength.toFixed(0)} 字符)`);
    } else {
        console.log(`📝 输出详细度: 旧架构更详细 (${oldStats.avgOutputLength.toFixed(0)} vs ${newStats.avgOutputLength.toFixed(0)} 字符)`);
    }

    // 总体建议
    console.log('\n📌 建议:');
    if (newStats.successCount >= oldStats.successCount && newStats.avgLatency <= oldStats.avgLatency * 1.2) {
