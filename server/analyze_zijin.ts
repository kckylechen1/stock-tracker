 ...
 * 分析单日是否满足启动条件
 */
function checkLaunchConditions(day: KlineData, prev20Days: KlineData[]): {
    isBreakout: boolean;
...
async function main() {
    console.log('╔════════════════════════════════════════════════════════════════════════════════╗');
    console.log('║   紫金矿业(601899)启动特征分析                                      ║');
    console.log('║   年度涨幅: +153.54% | 为什么信号系统未检测到？                      ║');
    console.log('╚═════════════════════════════════════════════════════════════════════════╝\n');
    ...
    const yearGain = ((current.close - yearStart!.close) / yearStart!.close) * 100;
    ...
    const candidates: Array<{
        ...
        changePct: number;
        volume: number;
        conditions: { isBreakout: boolean; isHighVolume: boolean; isBigUp: boolean; passAll: boolean };
        prev20High: number;
        prev20AvgVol: number;
    }> = [];
    ...
    for (let i = 60; i < klines.length; i++) {
        ...
        if (isBreakout && isHighVolume && isBigUp) {
            ...
            console.log(`   收盘价: ${day.close.toFixed(2)}元 (涨幅: ${changePct.toFixed(2)}%)`);
            console.log(`   突破20日高点: ${prev20High.toFixed(2)}元 → ${day.close.toFixed(2)}元 (${((day.close / prev20High - 1) * 100).toFixed(2)}%)`);
            console.log(`   成交量: ${formatVolume(day.volume)} (${(day.volume / prev20AvgVol).toFixed(1)}x平均)`);
            return;
        }
    }
    ...
    candidates.slice(-10).forEach(c => {
        ...
        console.log(`${c.date}`);
        console.log(`   价格: ${c.price.toFixed(2)}元 (日涨幅: ${c.changePct > 0 ? '+' : ''}${c.changePct.toFixed(2)}%)`);
    ...
    sortedByGain.slice(0, 5).forEach((k, idx) => {
        ...
        console.log(`  ${idx + 1}. ${k.date}: ${gain.toFixed(2)}% (${k.open.toFixed(2)} → ${k.close.toFixed(2)})`);
    });
    ...
    sortedByVol.slice(0, 5).forEach((k, idx) => {
        const gain = ((k.close - k.open) / k.open) * 100;
        console.log(`  ${idx + 1}. ${k.date}: ${formatVolume(k.volume)} (涨幅: ${gain.toFixed(2)}%)`);
    });
