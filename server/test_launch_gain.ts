 ...
 * 计算持有期间的最大涨幅和回撤
 */
function calculateHoldStats(klines: KlineData[], startDate: string): {
    maxGain: number;
    maxDrawdown: number;
    days: number;
} {
 ...
 * 测试单只股票
 */
async function testStock(symbol: string, name: string): Promise<TestResult | null> {
    try {
        ...
        const yearGain = yearStart ? ((current.close - yearStart.close) / yearStart.close) * 100 : 0;
        ...
        if (launchDate) {
            ...
            if (launchIdx >= 0) {
                ...
                const indicators = calculateIndicators(
                    launchLatest, closes, highs, lows, volumes
                );
        ...
        // 计算从启动日到现在的涨幅
        const launchToNowGain = launchDate ? calculateGainFromDay(klines, launchDate) : null;
        
        // 计算持有期间统计
        const holdStats = launchDate ? calculateHoldStats(klines, launchDate) : {
            maxGain: null,
            maxDrawdown: null,
            days: null,
        };
        ...
        if (launchDate) {
            ...
            if (week1Idx > launchIdx) {
                groups.week1 = ((klines[week1Idx].close - klines[launchIdx].close) / klines[launchIdx].close) * 100;
            }
            
            // 2周后（10个交易日）
            const week2Idx = Math.min(klines.length - 1, launchIdx + 10);
            if (week2Idx > launchIdx) {
                groups.week2 = ((klines[week2Idx].close - klines[launchIdx].close) / klines[launchIdx].close) * 100;
            }
            
            // 4周后（20个交易日）
            const week4Idx = Math.min(klines.length - 1, launchIdx + 20);
            if (week4Idx > launchIdx) {
                groups.week4 = ((klines[week4Idx].close - klines[launchIdx].close) / klines[launchIdx].close) * 100;
            }
            
            // 3个月后（60个交易日）
            const month3Idx = Math.min(klines.length - 1, launchIdx + 60);
            if (month3Idx > launchIdx) {
                groups.month3 = ((klines[month3Idx].close - klines[launchIdx].close) / klines[launchIdx].close) * 100;
            }
        }
...
async function main() {
    console.log('╔════════════════════════════════════════════════════════════════════════════════╗');
    console.log('║   从启动日开始涨幅回测                                             ║');
    console.log('║   验证：检测到启动信号后，后续涨幅如何？                        ║');
    console.log('╚═══════════════════════════════════════════════════════════════════════╝\n');
    ...
    for (let i = 0; i < testCases.length; i++) {
        ...
        console.log(`[${i + 1}/${testCases.length}] 分析 ${testCase.name}(${testCase.symbol})...`);
    ...
    const detectedNowGains = detected.filter(r => r.launchToNowGain !== null).map(r => r.launchToNowGain!);
    if (detectedNowGains.length > 0) {
        console.log(`  启动后平均涨幅: ${(detectedNowGains.reduce((a, b) => a + b, 0) / detectedNowGains.length).toFixed(2)}%`);
        console.log(`  启动后最高涨幅: ${Math.max(...detectedNowGains).toFixed(2)}%`);
        console.log(`  启动后最低涨幅: ${Math.min(...detectedNowGains).toFixed(2)}%`);
        
        const positiveCount = detectedNowGains.filter(g => g > 0).length;
        console.log(`  盈利比例: ${positiveCount}/${detectedNowGains.length} (${(positiveCount / detectedNowGains.length * 100).toFixed(1)}%)`);
    }
    
    const detectedMaxGains = detected.filter(r => r.holdStats.maxGain !== null).map(r => r.holdStats.maxGain!);
    if (detectedMaxGains.length > 0) {
        console.log(`  持有期间平均最大涨幅: ${(detectedMaxGains.reduce((a, b) => a + b, 0) / detectedMaxGains.length).toFixed(2)}%`);
    }
    ...
    timePoints.forEach(tp => {
        const gains = detected.filter(r => r.groups[tp.key] !== null).map(r => r.groups[tp.key]!);
        if (gains.length > 0) {
            ...
            console.log(`  盈利比例: ${positive}/${gains.length} (${(positive / gains.length * 100).toFixed(1)}%)`);
            console.log('');
        }
    });
    ...
    const sorted = results.filter(r => r.launchToNowGain !== null).sort((a, b) => (b.launchToNowGain || 0) - (a.launchToNowGain || 0));

    sorted.forEach((r, idx) => {
        const status = r.detected ? '✅' : '❌';
        const gain = r.launchToNowGain !== null ? (r.launchToNowGain > 0 ? '+' : '') + r.launchToNowGain.toFixed(2) + '%' : 'N/A';
        const maxGain = r.holdStats.maxGain !== null ? '+' + r.holdStats.maxGain.toFixed(2) + '%' : 'N/A';
        const drawdown = r.holdStats.maxDrawdown !== null ? r.holdStats.maxDrawdown.toFixed(2) + '%' : 'N/A';
        const days = r.holdStats.days !== null ? r.holdStats.days + '天' : 'N/A';
        
        console.log(`${idx + 1}. ${status} ${r.name}(${r.symbol})`);
        console.log(`   启动日: ${r.launchDate || 'N/A'} | 启动价格: ${r.launchPrice.toFixed(2)}元`);
        console.log(`   当前价格: ${r.currentPrice.toFixed(2)}元 | 启动后涨幅: ${gain}`);
        console.log(`   持有期间最高涨幅: ${maxGain} | 最大回撤: ${drawdown} | 持有天数: ${days}`);
