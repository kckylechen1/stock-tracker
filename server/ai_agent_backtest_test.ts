...
async function validateStockData(stockCode: string, testDate: string): Promise<boolean> {
  try {
    // 检查历史数据 (至少50个交易日)
    const historicalData = await akshare.getStockHistory(stockCode, 'daily', 90);
...
// 重新随机抽取替换股票
async function getRandomReplacementStock(excludeCode: string): Promise<string | null> {
  const random = seededRandom(Date.now());

  for (let i = 0; i < 100; i++) { // 最多尝试100次
    ...
    const randomCode = Math.floor(random() * (pool.range[1] - pool.range[0] + 1)) + pool.range[0];
    const stockCode = randomCode.toString().padStart(6, '0');

    if (stockCode !== excludeCode && await validateStockData(stockCode, TEST_CONFIG.testDate)) {
      return stockCode;
    }
  }
...
async function testSingleStock(stockCode: string, model: 'grok' | 'glm'): Promise<TestResult> {
  ...
  try {
    ...
    // 执行回测验证
    const backtestResult = await performBacktest(stockCode, recommendation, TEST_CONFIG.testDate, TEST_CONFIG.backtestDays);
...
function parseRecommendation(content: string): { type: '买入' | '持有' | '卖出'; confidence: number; reasoning: string } {
  ...
  if (content.includes('强烈买入') || content.includes('推荐买入') || content.includes('积极买入')) {
    ...
    type = '卖出';
    confidence = 60;
  } else if (content.includes('持有') || content.includes('观望') || content.includes('等待')) {
    type = '持有';
    confidence = 50;
  }
...
async function performBacktest(
  ...
  testDate: string,
  backtestDays: number
): Promise<{ totalReturn: number; maxGain: number; maxLoss: number; accuracy: number; valid: boolean }> {
  try {
    // 获取后续数据
    const futureData = await akshare.getStockHistory(stockCode, 'daily', backtestDays * 2);
    const testDateTime = new Date(testDate);

    // 过滤出测试日期之后的数据
    const backtestData = futureData.filter(d => new Date(d.date) > testDateTime).slice(0, backtestDays);

    if (!backtestData || backtestData.length < backtestDays / 2) {
      return { totalReturn: 0, maxGain: 0, maxLoss: 0, accuracy: 0, valid: false };
    }
...
// 生成测试报告
function generateTestReport(grokResults: TestResult[], glmResults: TestResult[]): string {
  // 计算统计数据
...
// 生成详细示例
function generateDetailedExamples(grokResults: TestResult[], glmResults: TestResult[]): string {
  let content = '\n## 详细结果示例\n\n';

  // 成功案例
  content += '### 成功预测案例\n\n';
  const successExamples = grokResults.filter(r =>
  ...
  // 失败案例
  content += '### 预测偏差案例\n\n';
  const failExamples = grokResults.filter(r =>
...
// 生成错误分析
function generateErrorAnalysis(grokResults: TestResult[], glmResults: TestResult[]): string {
...
// 生成结论
function generateConclusion(grokStats: any, glmStats: any, comparison: any): string {
  let content = '\n## 结论与建议\n\n';
  ...
  content += `### 部署建议\n`;
  if (Math.max(grokStats.successRate, glmStats.successRate) >= 85 &&
...
async function main() {
  ...
  try {
    ...
    if (validStocks.length < 80) {
      throw new Error(`有效股票数量不足 (${validStocks.length}/100)，无法继续测试`);
    }
