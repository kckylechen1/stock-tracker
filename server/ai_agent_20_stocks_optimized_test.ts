...
const OPTIMIZED_STOCKS = [
  // 上海主板 - 8只
  '600000', '600036', '600519', '600276', '600036', '600276', '600519', '600036',
  // 深圳主板 - 7只
  '000001', '000002', '000858', '000001', '000002', '000858', '000001',
  // 创业板 - 3只
  '300750', '300274', '300750',
  // 科创板 - 2只
  '688981', '688008'
].slice(0, 20); // 确保正好20只

// 简化测试流程
async function testStockOptimized(stockCode: string, model: 'grok' | 'glm'): Promise<any> {
...
function parseSimpleRecommendation(content: string): string {
  if (content.includes('强烈买入') || content.includes('推荐买入')) return '强烈买入';
...
async function main() {
  ...
  for (let i = 0; i < OPTIMIZED_STOCKS.length; i += OPTIMIZED_CONFIG.concurrency) {
    const batch = OPTIMIZED_STOCKS.slice(i, i + OPTIMIZED_CONFIG.concurrency);
    console.log(`批次 ${Math.floor(i/OPTIMIZED_CONFIG.concurrency) + 1}: ${batch.join(', ')}`);
    ...
    if (i + OPTIMIZED_CONFIG.concurrency < OPTIMIZED_STOCKS.length) {
      ...
      await new Promise(resolve => setTimeout(resolve, OPTIMIZED_CONFIG.batchDelay));
    }
  }
  ...
  for (let i = 0; i < OPTIMIZED_STOCKS.length; i += OPTIMIZED_CONFIG.concurrency) {
    ...
    const batchPromises = batch.map(stock => testStockOptimized(stock, 'deepseek'));
    const batchResults = await Promise.all(batchPromises);
    allResults.push(...batchResults);
