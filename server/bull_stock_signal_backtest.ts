...
function calculateIndicators(
    ...
    if (rsiValue > 65 && rsiValue < 80) score += 20; // 强势但不超买
    else if (rsiValue > 50 && rsiValue <= 65) score += 15; // 偏强
...
function evaluateSignals(score: number, signals: string[]): any {
    ...
    const signalCount = [maSignal, macdSignalFlag, rsiSignalFlag, kdjSignalFlag, volumeSignalFlag].filter(Boolean).length;
...
async function analyzeStockAtDate(
    ...
    try {
        ...
        if (!klines || klines.length < 60) {
            console.log(`⚠️ ${symbol}: 历史数据不足（${klines?.length ?? 0}天，需要60天）`);
            return null;
        }

        const targetIdx = klines.findIndex(k => k.date === targetDate || k.date.startsWith(targetDate.split('T')[0]));
        if (targetIdx < 30) {
            console.log(`⚠️ ${symbol}: 目标日期 ${targetDate} 数据不足（需要30天历史）`);
            return null;
        }
        ...
        const totalGain = ((endData.close - startData.close) / startData.close) * 100;
        ...
        const preLaunchAnalysis = calculateIndicators(
            preLaunchLatest, preCloses, preHighs, preLows, preVolumes
        );
        ...
        const dayAfterAnalysis = calculateIndicators(
            dayAfterLatest, dayAfterCloses, dayAfterHighs, dayAfterLows, dayAfterVolumes
        );
        ...
        return {
            ...
            evaluation: {
                preLaunch: evaluateSignals(preLaunchAnalysis.gaugeScore, preLaunchAnalysis.signals),
                launchDay: evaluateSignals(launchDayAnalysis.gaugeScore, launchDayAnalysis.signals),
                dayAfterLaunch: evaluateSignals(dayAfterAnalysis.gaugeScore, dayAfterAnalysis.signals),
            },
        };
    } catch (error) {
...
function formatAnalysis(result: SignalAnalysis): string {
    const { symbol, name, startDate, endDate, startPrice, endPrice, totalGain, preLaunch, launchDay, dayAfterLaunch, evaluation } = result;
...
async function getStockList(): Promise<string[]> {
    const stocks: string[] = [];
    const commonSymbols = [
        '000001', '000002', '000858', '002594', '600000', '600036', '600519',
        '601318', '601939', '601988', '688111', '300750', '300502', '301308'
    ];
...
async function scanMarketForBullStocks(symbols: string[]): Promise<any[]> {
    ...
    for (const symbol of symbols) {
        try {
            ...
            const launchIdx = klines.findIndex(k => k.date === launchDate || k.date.startsWith(launchDate.split('T')[0]));
            ...
            const currentGain = ((latest.close - launchDay.close) / launchDay.close) * 100;
...
function displaySellAdvice(result: IndicatorResult, launchLow: number): string {
    ...
    if (result.sellSignals.length === 0 && result.totalScore >= 30 && result.price >= launchLow) {
        advice.push('\n✅ 暂无卖出信号，可继续持有');
    }
...
async function mainBacktest() {
    ...
    for (const testCase of testCases) {
        ...
        const result = await analyzeStockAtDate(testCase.symbol, testCase.startDate);
        if (result) {
            results.push(result);
            console.log(`✅ 分析完成 - 涨幅: ${result.totalGain > 0 ? '+' : ''}${result.totalGain.toFixed(2)}%`);
        } else {
            console.log(`❌ 分析失败`);
            failedCases.push({ symbol: testCase.symbol, name: testCase.name, error: '未知错误' });
        }
    }
    ...
    const validSignalsPreLaunch = results.filter(r => r.evaluation.preLaunch.overallSignal).length;
    const validSignalsLaunchDay = results.filter(r => r.evaluation.launchDay.overallSignal).length;
    const validSignalsDayAfter = results.filter(r => r.evaluation.dayAfterLaunch.overallSignal).length;
    const strongSignalsPreLaunch = results.filter(r => r.evaluation.preLaunch.strength === '强烈' || r.evaluation.preLaunch.strength === '明显').length;
    const strongSignalsLaunchDay = results.filter(r => r.evaluation.launchDay.strength === '强烈' || r.evaluation.launchDay.strength === '明显').length;
    const strongSignalsDayAfter = results.filter(r => r.evaluation.dayAfterLaunch.strength === '强烈' || r.evaluation.dayAfterLaunch.strength === '明显').length;
    const avgGain = results.reduce((sum, r) => sum + r.totalGain, 0) / results.length;
    const avgScorePreLaunch = results.reduce((sum, r) => sum + r.preLaunch.gaugeScore, 0) / results.length;
    const avgScoreLaunchDay = results.reduce((sum, r) => sum + r.launchDay.gaugeScore, 0) / results.length;
    const avgScoreDayAfter = results.reduce((sum, r) => sum + r.dayAfterLaunch.gaugeScore, 0) / results.length;
    ...
    console.log('\n  启动前信号: 有效率');
    console.log(`  有效信号: ${validSignalsPreLaunch}/${totalTests} (${(validSignalsPreLaunch / totalTests * 100).toFixed(0)}%)`);
    console.log(`  强信号数: ${strongSignalsPreLaunch}/${totalTests}`);
    console.log(`  平均评分: ${avgScorePreLaunch.toFixed(0)}/100`);
    
    console.log('\n  启动当天信号: 有效率');
    console.log(`  有效信号: ${validSignalsLaunchDay}/${totalTests} (${(validSignalsLaunchDay / totalTests * 100).toFixed(0)}%)`);
    console.log(`  强信号数: ${strongSignalsLaunchDay}/${totalTests}`);
    console.log(`  平均评分: ${avgScoreLaunchDay.toFixed(0)}/100`);

    console.log('\n  启动后1天信号: 有效率');
    console.log(`  有效信号: ${validSignalsDayAfter}/${totalTests} (${(validSignalsDayAfter / totalTests * 100).toFixed(0)}%)`);
    ...
    if (highGainers.length > 0) {
        ...
        highGainers.forEach(r => {
            ...
            console.log(`     启动当天评分: ${r.launchDay.gaugeScore.toFixed(0)}/100`);
    ...
    // 信号有效性分析
    console.log('\n📊 信号有效性分析\n');
    console.log(`1. 启动前有效信号率: ${(validSignalsPreLaunch / totalTests * 100).toFixed(0)}%`);
    console.log(`2. 启动当天有效信号率: ${(validSignalsLaunchDay / totalTests * 100).toFixed(0)}%`);
    console.log(`3. 启动后1天有效信号率: ${(validSignalsDayAfter / totalTests * 100).toFixed(0)}%`);
    ...
    Object.entries(signalCountPre)
        ...
        .forEach(([signal, count]) => {
            console.log(`  ${signal}: ${count}/${results.length} (${(count / results.length * 100).toFixed(0)}%)`);
        });
...
async function mainScan() {
    ...
    console.log('═'.repeat(80));
    console.log(`📊 扫描完成，共分析 ${symbols.length} 只股票，发现 ${opportunities.length} 个机会\n`);
}
...
if (mode === 'scan') {
    ...
    console.log('使用方法:');
    console.log('  回测模式: npx tsx server/bull_stock_signal_backtest.ts backtest');
