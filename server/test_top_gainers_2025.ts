...
async function main() {
    console.log('╔════════════════════════════════════════════════════════════════════════════════╗');
    console.log('║   2025年涨幅最大的30只股票（市值>200亿）牛股信号回测              ║');
    console.log('║   目标：验证信号系统能否识别这些牛股的启动点                    ║');
    console.log('╚═════════════════════════════════════════════════════════════════════════╝\n');
    ...
    for (let i = 0; i < testCases.length; i++) {
        ...
        if (result) {
            ...
            console.log(`  ${status} 年度涨幅: ${gain}${result.yearGain.toFixed(2)}% | 启动评分: ${score}`);
        } else {
    ...
    const avgScore = results.filter(r => r.launchScore !== null).reduce((sum, r) => sum + (r.launchScore ?? 0), 0) / total;
    
    const detectedResults = results.filter(r => r.detected);
    const avgGainDetected = detectedResults.length > 0 
        ? detectedResults.reduce((sum, r) => sum + r.yearGain, 0) / detectedResults.length 
        : 0;
    
    const undetectedResults = results.filter(r => !r.detected);
    const avgGainUndetected = undetectedResults.length > 0 
        ? undetectedResults.reduce((sum, r) => sum + r.yearGain, 0) / undetectedResults.length 
        : 0;

    console.log(`总测试数: ${total}`);
    console.log(`成功识别: ${detected} (${(detected / total * 100).toFixed(1)}%)`);
    ...
    results.forEach((r, idx) => {
        ...
        console.log(`   年度涨幅: ${gain}${r.yearGain.toFixed(2)}% | 启动日期: ${launch} | 启动评分: ${score}`);
