...
class TestAgent extends BaseAgent {
    ...
    constructor(modelType: 'grok' | 'glm') {
        ...
        const testTools: ToolDefinition[] = stockTools.filter(t =>
            ['get_stock_quote', 'analyze_stock_technical', 'get_fund_flow', 'get_current_datetime'].includes(t.function.name)
        ) as ToolDefinition[];
    ...
    private registerTestTools(): void {
        const toolNames = ['get_stock_quote', 'analyze_stock_technical', 'get_fund_flow', 'get_current_datetime'];
    ...
    protected override async callLLM() {
        ...
        const response = await fetch(`${this.modelConfig.apiUrl}/chat/completions`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`${this.modelConfig.name} Error: ${response.status} - ${error.slice(0, 200)}`);
        }
...
const TEST_CASES = [
    ...
    },
    {
        ...
        expectedTools: ['get_stock_quote', 'analyze_stock_technical', 'get_fund_flow'],
    },
    {
        name: '时间感知',
...
async function runComparisonTest() {
    ...
    for (const testCase of TEST_CASES) {
        ...
        if (grokResult.success) {
            console.log(`      ✅ 成功 | 延迟: ${grokResult.latency}ms | 工具: ${grokResult.toolsUsed.join(', ') || '无'}`);
        ...
        if (glmResult.success) {
            console.log(`      ✅ 成功 | 延迟: ${glmResult.latency}ms | 工具: ${glmResult.toolsUsed.join(', ') || '无'}`);
    ...
    const summarize = (modelResults: TestResult[], name: string) => {
        ...
        const avgLatency = modelResults.filter(r => r.success).reduce((acc, r) => acc + r.latency, 0) / successCount || 0;
        const totalToolCalls = modelResults.reduce((acc, r) => acc + r.toolCallCount, 0);
        const avgOutputLength = modelResults.filter(r => r.success).reduce((acc, r) => acc + r.outputLength, 0) / successCount || 0;

        console.log(`【${name}】`);
        console.log(`   成功率: ${successCount}/${modelResults.length} (${(successCount / modelResults.length * 100).toFixed(0)}%)`);
    ...
    // 对比表格
    console.log('📋 详细对比表\n');
    console.log('| 测试用例 | Grok 成功 | Grok 延迟 | Grok 工具数 | GLM 成功 | GLM 延迟 | GLM 工具数 |');
    console.log('|---------|----------|----------|------------|---------|---------|-----------|');

    for (const testCase of TEST_CASES) {
        const grok = results.find(r => r.model === 'Grok' && r.testCase === testCase.name)!;
        const glm = results.find(r => r.model === 'GLM' && r.testCase === testCase.name)!;

        console.log(`| ${testCase.name.padEnd(8)} | ${grok.success ? '✅' : '❌'}       | ${String(grok.latency).padStart(7)}ms | ${String(grok.toolCallCount).padStart(10)} | ${glm.success ? '✅' : '❌'}      | ${String(glm.latency).padStart(6)}ms | ${String(glm.toolCallCount).padStart(9)} |`);
    }
    ...
    const grokSuccessRate = grokResults.filter(r => r.success).length / grokResults.length;
    const glmSuccessRate = glmResults.filter(r => r.success).length / glmResults.length;
    const grokAvgLatency = grokResults.filter(r => r.success).reduce((acc, r) => acc + r.latency, 0) / grokResults.filter(r => r.success).length || Infinity;
    const glmAvgLatency = glmResults.filter(r => r.success).reduce((acc, r) => acc + r.latency, 0) / glmResults.filter(r => r.success).length || Infinity;
    const grokTotalTools = grokResults.reduce((acc, r) => acc + r.toolCallCount, 0);
    const glmTotalTools = glmResults.reduce((acc, r) => acc + r.toolCallCount, 0);

    console.log(`Tool Call 稳定性: ${grokSuccessRate >= glmSuccessRate ? '🟢 Grok' : '🔵 GLM'} 更稳定 (${Math.max(grokSuccessRate, glmSuccessRate) * 100}%)`);
    console.log(`响应速度: ${grokAvgLatency <= glmAvgLatency ? '🟢 Grok' : '🔵 GLM'} 更快 (${Math.min(grokAvgLatency, glmAvgLatency).toFixed(0)}ms)`);
    console.log(`工具调用积极性: ${grokTotalTools >= glmTotalTools ? '🟢 Grok' : '🔵 GLM'} 更积极 (${Math.max(grokTotalTools, glmTotalTools)}次)`);
}
...
async function runStabilityTest() {
    ...
    for (const modelType of ['grok', 'glm'] as const) {
        ...
        for (let i = 0; i < runs; i++) {
            ...
            console.log(`      ${result.success ? '✅' : '❌'} | ${result.latency}ms | 工具: ${result.toolsUsed.join(', ') || '无'}`);
        }
        ...
        const latencyVariance = latencies.length > 1
            ? Math.sqrt(latencies.reduce((acc, l) => acc + Math.pow(l - avgLatency, 2), 0) / latencies.length)
            : 0;
