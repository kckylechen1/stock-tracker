...
export function calculateIndicators(klines: KlineData[]): TechnicalIndicators {
  ...
  const latestBoll = bollResult[bollResult.length - 1] || {
    upper: undefined,
    middle: undefined,
    lower: undefined,
  };
...
export function calculateGaugeScore(klines: KlineData[]): {
  ...
  const k1 =
    Math.sign(trendScores.macdScore) === Math.sign(trendScores.emaScore)
      ? 1.2
      : 0.6;

  // K2：动量一致性 (RSI vs KDJ)
  const k2 =
    Math.sign(momentumScores.rsiScore) === Math.sign(momentumScores.kdjScore)
      ? 1.15
      : 0.7;

  // K3：量能一致性 (OBV vs VR) - 最重要
  const k3 =
    Math.sign(volumeScores.obvScore) === Math.sign(volumeScores.vrScore)
      ? 1.3
      : 0.5;
  ...
  const confidence =
    (Math.max(0, sTrend) * 0.25 +
      ...
      Math.max(0, sVolatility) * 0.2 +
      Math.max(0, sVolume) * 0.3) /
    100;
