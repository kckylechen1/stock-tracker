 ...
 * @returns 回测结果
 */
export function runBacktest(klines: KlineData[], symbol: string): BacktestResult {
    ...
    for (let i = 60; i < klines.length - 5; i++) {
        ...
        const return1Day = price1Day ? ((price1Day - currentPrice) / currentPrice) * 100 : null;
        const return5Day = price5Day ? ((price5Day - currentPrice) / currentPrice) * 100 : null;

        // 判断预测是否正确
        // Buy/Strong Buy 信号：期望价格上涨
        // Sell/Strong Sell 信号：期望价格下跌
        const isBuySignal = result.signal === 'Buy' || result.signal === 'Strong Buy';
        const isCorrect = return5Day !== null && (
            (isBuySignal && return5Day > 0) ||
            (!isBuySignal && return5Day < 0)
        );
...
export function formatBacktestReport(result: BacktestResult): string {
    const { symbol, startDate, endDate, totalSignals, correctPredictions, accuracy, avgReturn, maxDrawdown } = result;
