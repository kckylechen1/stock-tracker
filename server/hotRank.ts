 ...
 */
export async function getStockHotRank(symbol: string): Promise<HotRankResult | null> {
    try {
 ...
 * @param symbol 股票代码
 */
export async function getStockHotRankHistory(symbol: string): Promise<HotRankHistory[]> {
    try {
        // 格式化代码
        let formattedSymbol = symbol;
        if (!symbol.startsWith('SZ') && !symbol.startsWith('SH')) {
...
export function analyzeHotRankTrend(history: HotRankHistory[]): {
    ...
    const olderAvg = olderRanks.length > 0
