...
async function testMarketBreadth() {
    ...
    console.log(`   总数 = 上涨+下跌+平盘: ${breadth.riseCount + breadth.fallCount + breadth.flatCount}`);
    console.log(`   是否一致: ${(breadth.riseCount + breadth.fallCount + breadth.flatCount) === breadth.totalCount ? '✅ 是' : '❌ 否'}`);
    console.log(`   上涨比例是否合理: ${breadth.riseRatio > 0 && breadth.riseRatio < 100 ? '✅ 是' : '❌ 否'}`);
}

testMarketBreadth().then(() => {
    console.log('');
    console.log('=== 测试完成 ===');
    process.exit(0);
}).catch(error => {
