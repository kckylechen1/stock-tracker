 ...
 * 获取AI应用板块的成分股
 */
async function getAIApplicationStocks(
  boardCode: string
): Promise<ConstituentStock[]> {
  try {
    ...
    const aStocks = stocks.filter(stock => {
      const code = stock["代码"] || "";
      return (
        code.startsWith("0") || code.startsWith("3") || code.startsWith("6")
      );
    });
...
function detectLaunchDay(klines: any[]): string | null {
  for (let i = 60; i < klines.length; i++) {
    ...
    const prev20AvgVol =
      prev20Days.reduce((sum: number, k: any) => sum + k.volume, 0) / 20;
...
async function main() {
  ...
  for (let i = 0; i < aiStocks.length; i++) {
    ...
    if (result) {
      ...
      console.log(
        `  ${status} 年度涨幅: ${result.yearGain > 0 ? "+" : ""}${result.yearGain.toFixed(2)}%`
      );
    } else {
  ...
  console.log(
    `  信号识别: ${detected}只 (${((detected / total) * 100).toFixed(1)}%)`
  );
  ...
  topGainers.forEach((stock, index) => {
    ...
    console.log(
      `  ${index + 1}. ${status} ${stock.name}(${stock.symbol}): ${stock.yearGain > 0 ? "+" : ""}${stock.yearGain.toFixed(2)}%`
    );
  });
  ...
  if (signalStocks.length > 0) {
    ...
    signalStocks.forEach(stock => {
      console.log(
        `  ✅ ${stock.name}(${stock.symbol}): +${stock.yearGain.toFixed(2)}% | 启动日: ${stock.launchDate}`
      );
    });
  }
