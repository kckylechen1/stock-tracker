...
function calculateKDJ(
  ...
  for (let i = period - 1; i < closes.length; i++) {
    ...
    const k =
      kValues.length === 0 ? rsv : (kValues[kValues.length - 1] * 2 + rsv) / 3;
    const d =
      dValues.length === 0 ? k : (dValues[dValues.length - 1] * 2 + k) / 3;
    const j = 3 * k - 2 * d;
...
function calculateSignalScore(indicators: IndicatorResult): number {
  ...
  // 成交量评分 (最高15分)
  if (indicators.volumeStatus === "放量" && indicators.priceChange > 0)
    score += 15;
